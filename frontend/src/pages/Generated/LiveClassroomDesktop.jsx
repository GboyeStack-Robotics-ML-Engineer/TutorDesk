...
// Live in-app classroom.
//
// Shows that a full class runs inside TutorDesk itself: video tiles, mic /
// camera / screen-share / whiteboard / chat controls, participant list, the
// shareable class link, an elapsed timer, and an "End class" action that leads
// to the post-class wrap. This is the screen that proves the app contains all
// the functions of a call, not just a link out to Zoom.
...
export const LiveClassroomDesktop = () => {
  ...
  return (
    <div className="flex flex-col gap-space-4" style={{ minHeight: '78vh' }}>
      {/* Top bar: class meta + live pill + link */}
      <div className="flex flex-wrap items-center justify-between gap-space-3">
        <div className="flex items-center gap-space-3">
          <span className="inline-flex items-center gap-1.5 px-space-3 py-1 rounded-full bg-danger-tint text-danger-solid font-label text-caption font-semibold">
            <span className="w-2 h-2 rounded-full bg-danger-solid animate-pulse" />
            LIVE
          </span>
          <div>
            <h1 className="font-heading text-title-md text-ink-900 leading-tight">
              WAEC Mathematics — Tunde Okafor
            </h1>
            <p className="font-caption text-caption text-ink-500">
              Algebra: simultaneous equations · 1-on-1 session
            </p>
          </div>
        </div>
        <div className="flex items-center gap-space-3">
          ...
          <button
            onClick={copyLink}
            className="flex items-center gap-2 bg-surface-container-low hover:bg-surface-container border border-paper-300 px-space-3 py-1.5 rounded-lg font-label text-label text-ink-700 transition-colors"
          >
            <span className="material-symbols-outlined text-[18px]">
              {copied ? 'check' : 'link'}
            </span>
            {copied ? 'Link copied' : 'meet.tutordesk.ng/c/8fk2'}
          </button>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col gap-space-3">
          ...
          <div className="relative flex-1 rounded-xl overflow-hidden bg-[#0f1c1a] flex items-center justify-center" style={{ minHeight: 340 }}>
            ...
            <div className="absolute bottom-3 right-3 w-40 h-28 rounded-lg overflow-hidden bg-[#12302b] border border-white/10 flex items-center justify-center">
              ...
              <div className="absolute bottom-1.5 left-1.5 text-white/90 font-label text-[10px] bg-black/40 px-2 py-0.5 rounded">
                You (Aisha)
              </div>
            </div>
          </div>
          ...
          {/* Control bar */}
          <div className="flex items-center justify-center gap-space-3 bg-surface-container-low rounded-xl py-space-3 px-space-4">
            {controls.map((c) => {
              const active = state[c.key];
              const isToggleOffStyle = (c.key === 'mic' || c.key === 'cam') && !active;
              return (
                <button
                  key={c.key}
                  ...
                  className={`flex flex-col items-center gap-1 px-space-3 py-2 rounded-lg transition-all ${
                    isToggleOffStyle
                      ? 'bg-danger-tint text-danger-solid'
                      : active && (c.key === 'share' || c.key === 'board' || c.key === 'hand')
            ...
            <button
              ...
              title="Chat"
              className={`flex flex-col items-center gap-1 px-space-3 py-2 rounded-lg transition-all ${
                chatOpen ? 'bg-primary-container text-on-primary-container' : 'text-ink-700 hover:bg-surface-container'
              }`}
            >
            ...
            <button
              onClick={() => navigate('/portal/view/classroom-post-class-wrap')}
              title="End class"
              className="flex items-center gap-2 bg-danger-solid text-white px-space-4 py-2.5 rounded-lg font-label text-label hover:opacity-90 transition-opacity ml-1"
            >
        ...
        {chatOpen && (
          <div className="w-80 flex flex-col gap-space-3" style={{ minWidth: 300 }}>
            ...
            <div className="bg-paper-0 border border-paper-200 rounded-xl flex-1 flex flex-col overflow-hidden" style={{ minHeight: 240 }}>
              <div className="px-space-4 py-space-3 border-b border-paper-200">
                <h3 className="font-label text-label text-ink-700">In-class chat</h3>
              </div>
              <div className="flex-1 overflow-y-auto p-space-4 flex flex-col gap-space-3">
                {messages.map((m, i) => (
                  <div key={i} className={`flex flex-col ${m.me ? 'items-end' : 'items-start'}`}>
                    ...
                    <div className={`px-space-3 py-2 rounded-lg font-body text-body max-w-[85%] ${
                      m.me ? 'bg-primary text-on-primary' : 'bg-surface-container-low text-ink-900'
                    }`}>
                      {m.text}
                    </div>
                  </div>
